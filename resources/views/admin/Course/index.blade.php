...
@section('breadcrumb')
    <h1 class="d-flex text-dark fw-bolder my-1 fs-3">{{__('lang.Courses')}}</h1>
    <!--end::Title-->
    <!--begin::Breadcrumb-->
                ...
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-4 gy-5" id="users_table">
                        ...
                        <tr class="text-start text-muted fw-bolder fs-5 text-uppercase gs-0">
                            ...
                            <th class="min-w-125px">{{__('lang.is_popular')}} </th>
                            <th class="min-w-125px">{{__('lang.MainCategory')}} </th>
                            <th class="min-w-125px">{{__('lang.SubCategory')}} </th>
                            <th class="min-w-125px">  {{__('lang.created_by')}} </th>
                            <th class="min-w-125px">  {{__('lang.updated_by')}} </th>
                            <th class="min-w-125px">  {{__('lang.lessons')}} </th>
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('#users_table').DataTable({
                ...
                ajax: {
                    url: '{{ route('Course.datatable.data') }}',
                    data: {
                ...
                columns: [
                    ...
                    {data: 'is_popular', name: 'is_active', "searchable": true, "orderable": true},
                    {data: 'main_category_id', name: 'main_category_id', "searchable": true, "orderable": true},
                    {data: 'sub_category_id', name: 'sub_category_id', "searchable": true, "orderable": true},
                    ...
                    {data: 'lessons', name: 'lessons', "searchable": false, "orderable": false},
                    {data: 'actions', name: 'actions', "searchable": false, "orderable": false},

                ]
            });
            $.ajax({
                url: "{{ URL::to('/add-button-Course')}}",
